...
// Fixed toggleOriginalView function - add this to EditorUtils.jsx
export const toggleOriginalView = (
  ...
  console.log(`Toggling original view, current mode: ${mode}`);
  
  if (mode === 'brush') {
    // Switch from brush mode to original mode
    setMode('original');
    if (maskCanvasRef.current) {
    ...
    // Switch from original mode back to brush mode
    setMode('brush');
    if (maskCanvasRef.current) {
      maskCanvasRef.current.style.display = 'block';
    }
    ...
    if (nonOriginalItems.length > 0) {
      // If we have edited versions, load the most recent one
      ...
      console.log("Switched back to brush mode with last edited item");
    } else {
      // If no edited versions, load the original image in brush mode instead of staying in original mode
      // This fixes the "stuck in original mode" bug
      loadImageToCanvas(pendingImages[currentImageIndex]);
      
      // Set history to the original image
      if (history.length > 0) {
        const originalItem = history.find(item => item.isOriginal);
        if (originalItem) {
          setCurrentHistoryItem(originalItem);
          setHistoryIndex(history.findIndex(item => item.id === originalItem.id));
        }
      }
      console.log("Switched to brush mode with original image");
    }
  }
};
...
export const hideZoomControls = (setShowZoomControls) => {
  setShowZoomControls(false);
};

// New function to show the controls panel
export const showZoomControls = (setShowZoomControls) => {
  setShowZoomControls(true);
};

export const toggleZoomDrawMode = (zoomDrawMode, setZoomDrawMode, isMobile = false) => {
  const newMode = !zoomDrawMode;
  setZoomDrawMode(newMode);
...
export const handleZoomIn = (setZoomLevel, MAX_ZOOM_LEVEL, isZoomed, setIsZoomed, setShowZoomControls, alwaysShowControls = true) => {
  setZoomLevel(prev => {
    const newLevel = Math.min(prev + 1, MAX_ZOOM_LEVEL);
    if (newLevel > 0 && prev === 0 && alwaysShowControls) {
      setShowZoomControls(true);
    }
    return newLevel;
  });
