...
// Note: You should create or import these from your actual utility file
import { 
  StyleInjector, 
  Haptics, 
  MobileToast,
...
const BlurEditor_mobile = ({ initialImage, onReset }) => {
    ...
    // NEW FEATURE: Inverse Selection - ported from background removal
    const handleInvertSelection = () => {
        ...
        setToast({
            message: isSelectionInverted ? 
                "Selection mode: Keep subject in focus" : 
                "Selection mode: Blur the subject",
            type: "info"
        });
    };
    ...
    const handleGenerateMask = async () => {
        ...
        try {
            ...
            if (detectionMode === "manual" && drawingCanvasRef.current) {
                ...
                setToast({ 
                    message: isSelectionInverted ? 
                        "Mask generated: Areas selected will be blurred" : 
                        "Mask generated: Subject selected will stay in focus",
                    type: "success" 
                });
                Haptics.notification('success');
                return;
            ...
            if (!mask) {
                setError("Please draw on the image to mark the subject you want to keep in focus");
    ...
    const handleProcessImage = async () => {
        ...
        try {
            setLoadingStates(prev => ({ ...prev, blurProcessing: true }));
            setIsProcessing(true);
            setError(null);
            ...
            if (detectionMode === "auto") {
                // For auto mode, we do a one-step auto-blur
                ...
                const response = await fetch('http://localhost:8000/api/apply-blur', {
                    ...
                    body: JSON.stringify({
                        ...
                        mask: mask,
                        blur_strength: blurStrength,
                        invert_mask: isSelectionInverted
                    })
                });
                ...
                throw new Error("Please draw on the image or use auto detection first");
            }
        } catch (err) {
            setError(err.message);
            ...
            setLoadingStates(prev => ({ ...prev, blurProcessing: false }));
            setIsProcessing(false);
        }
    };
    ...
    // NEW FEATURE: Redo - ported from background removal
    ...
    // NEW FEATURE: Remaster - ported from background removal
    ...
    return (
        <div style={{
                    ...
                    <div style={{ marginBottom: '1rem' }}>
                        ...
                        <input
                            ...
                            onChange={(e) => {
                                ...
                                if (Math.abs(parseInt(e.target.value) - blurStrength) > 5) {
                                    Haptics.impact('light');
                                }
                            }}
                            style={{
                        ...
                        <p style={{
                            ...
                            {detectionMode === "manual" 
                                ? isSelectionInverted
                                    ...
                                    : "Draw on the image to mark the subject you want to keep in focus. Double-tap to reset zoom, pinch to zoom in/out."
                                : "Auto-detection will automatically identify the main subject and keep it in focus while blurring the background."}
                        </p>
                    </div>
                </div>
            )}
            ...
            <div style={{
                ...
                {/* Generate Mask Button for manual mode */}
                {detectionMode === "manual" && (
                    <button
                ...
                <button
                    ...
                    disabled={
                        isProcessing || 
                        (detectionMode === "manual" && !mask)
                    }
                    style={{
                        flex: 1,
                        padding: '0.875rem',
                        backgroundColor: (detectionMode === "manual" && !mask) || isProcessing
                            ? '#F3F4F6' 
                            : '#abf134',
                        color: (detectionMode === "manual" && !mask) || isProcessing
                            ? '#9CA3AF' 
                            : '#000000',
                        borderRadius: '0.5rem',
                        fontWeight: 600,
...
export default BlurEditor_mobile;